...
describe("decideCopyAction", () => {
  test("fixed buy uses amountSOL and applies perTradeCap and dailyCap remaining", () => {
    const followed = { mode: "fixed", amountSOL: 0.2, perTradeCapSOL: 0.1, dailyCapSOL: 0.15 };
    const res = decideCopyAction({ eventType: "buy", followed, dailySpent: 0.05, envDefaultDailyCap: 10 });
  ...
  test("percent buy uses dailyCap% and respects remaining", () => {
    ...
    const res = decideCopyAction({ eventType: "buy", followed, dailySpent: 0.9, envDefaultDailyCap: 5 });
  ...
  test("percent buy falls back to env default daily cap when not set", () => {
    ...
    const res = decideCopyAction({ eventType: "buy", followed, dailySpent: 0, envDefaultDailyCap: 2 });
    expect(res.execute).toBe(true);
  ...
  test("sell returns percent clamped to [1,100]", () => {
    ...
    const res = decideCopyAction({ eventType: "sell", followed, dailySpent: 0 });
    expect(res.execute).toBe(true);
  ...
  test("invalid or exhausted daily cap returns execute=false", () => {
    ...
    const r1 = decideCopyAction({ eventType: "buy", followed, dailySpent: 0.1 });
    expect(r1.execute).toBe(false);
    expect(r1.reason).toBe("daily_cap_exhausted");

    const r2 = decideCopyAction({ eventType: "buy", followed: { mode: "fixed", amountSOL: 0 }, dailySpent: 0 });
