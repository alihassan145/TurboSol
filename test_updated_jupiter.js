...
async function testUpdatedJupiterAPI() {
    ...
    for (const outputToken of testTokens) {
      ...
      try {
        ...
        if (quoteResult && quoteResult.route) {
          ...
          console.log(
            `  📈 Price Impact: ${quoteResult.route.priceImpactPct}%`
          );
          console.log(
            `  🛣️  Route Plan: ${
              quoteResult.route.routePlan?.length || 0
            } steps`
          );
        } else {
  ...
  try {
    ...
    if (quoteResult && quoteResult.route) {
      ...
      console.log(
        `    - inputMint: ${quoteResult.route.inputMint ? "✅" : "❌"}`
      );
      console.log(
        `    - outputMint: ${quoteResult.route.outputMint ? "✅" : "❌"}`
      );
      console.log(
        `    - inAmount: ${quoteResult.route.inAmount ? "✅" : "❌"}`
      );
      console.log(
        `    - outAmount: ${quoteResult.route.outAmount ? "✅" : "❌"}`
      );
      console.log(
        `    - routePlan: ${quoteResult.route.routePlan ? "✅" : "❌"}`
      );
      console.log(
        `    - priceImpactPct: ${
          quoteResult.route.priceImpactPct !== undefined ? "✅" : "❌"
        }`
      );
      console.log(
        `    - slippageBps: ${quoteResult.route.slippageBps ? "✅" : "❌"}`
      );
    } else {
