...
export async function performSwap({
  ...
  if (!riskBypass) {
    ...
    const risk = await riskCheckToken(outputMint, { requireLpLock, maxBuyTaxBps });
    if (!risk.ok) {
      throw new Error(`Risk check failed: ${risk.reasons?.join("; ") || "blocked"}`);
    }
  }
  ...
  if (chatId !== undefined && chatId !== null) {
    ...
    const cap = (s.tierCaps && s.tierCaps[tier] != null) ? Number(s.tierCaps[tier]) : Infinity;
    const today = new Date();
    const key = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2,'0')}-${String(today.getDate()).padStart(2,'0')}`;
    const spent = Number(s.dailySpend?.[key] || 0);
    const remaining = cap === Infinity ? Infinity : Math.max(0, cap - spent);
    if (remaining <= 0) throw new Error(`Daily cap reached for tier '${tier}' (${cap} SOL)`);
    if (amountSol > remaining) amountSol = remaining;
  }

  // Resolve slippage and fee
  const slippage = slippageBps ?? Number(process.env.DEFAULT_SLIPPAGE_BPS || 100);
  const shouldUsePrivateRelay = (usePrivateRelay != null) ? !!usePrivateRelay : ((chatId!=null) ? !!getUserState(chatId).enablePrivateRelay : false);

  // Multi-wallet split flow
  if (splitAcrossWallets || (walletsCount && walletsCount > 1)) {
    if (!(chatId !== undefined && chatId !== null)) throw new Error("Split buy requires user context (chatId)");
    ...
    if (useJitoBundle) {
      ...
      for (const w of selected) {
        ...
        const tokensOut = route?.outAmount && route?.outToken?.decimals != null ? (route.outAmount / 10 ** route.outToken.decimals) : undefined;
      ...
      try {
        for (const m of perWalletMeta) {
          const avgPrice = m.tokensOut ? (Number(perWalletSol) / m.tokensOut) : undefined;
          addPosition(chatId, {
            mint: outputMint,
          ...
          addTradeLog(chatId, { kind: "buy", mint: outputMint, sol: perWalletSol, txid: bundleUuid, latencyMs, slot });
        }
      } catch {}
    ...
    for (const w of selected) {
      try {
        ...
        let txid;
        try {
          txid = await sendTransactionRaced(tx, { skipPreflight: true, usePrivateRelay: shouldUsePrivateRelay });
        } catch (e) {
          rotateRpc("race failed");
          txid = await connection.sendRawTransaction(tx.serialize(), { skipPreflight: true });
        }
        const latencyMs = Date.now() - t0;
        const slot = await connection.getSlot().catch(() => undefined);
        try {
          const tokensOut = route?.outAmount && route?.outToken?.decimals != null ? (route.outAmount / 10 ** route.outToken.decimals) : undefined;
          const avgPrice = tokensOut ? (Number(perWalletSol) / tokensOut) : undefined;
          addPosition(chatId, {
            mint: outputMint,
          ...
          addTradeLog(chatId, { kind: "buy", mint: outputMint, sol: perWalletSol, txid, latencyMs, slot });
        } catch {}
    ...
    return { txids: results.filter(r=>r.ok).map(r=>r.txid), elapsedMs: elapsed };
  }
  ...
  try {
    if (chatId !== undefined && chatId !== null) {
      ...
      addPosition(chatId, {
        ...
        txid,
        status: "open",
        source: useJitoBundle ? "jito" : (shouldUsePrivateRelay ? "relay" : "rpc"),
        sendLatencyMs: latencyMs,
        slot,
      });
      addTradeLog(chatId, { kind: "buy", mint: outputMint, sol: amountSol, txid, latencyMs, slot });
    }
  } catch {}
  return { txid };
}
...
export async function performSell({
  ...
  const resp = await connection.getParsedTokenAccountsByOwner(owner, { mint: tokenPk });
  ...
  try {
    if (chatId !== undefined && chatId !== null) {
      addTradeLog(chatId, { kind: "sell", mint: tokenMint, amountRaw: amountRaw.toString(), solOut, txid, latencyMs, slot });
    }
  } catch {}
  return { txid };
}
