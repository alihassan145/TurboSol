...
function asNumberInRange(name, v, { min = -Infinity, max = Infinity, def = undefined, errors }) {
...
function redactMongoAuth(str) {
  try {
    return str.replace(/(mongodb(?:\+srv)?:\/\/)([^@\n]+)@/gi, (m, p1) => `${p1}***@`);
  } catch { return str; }
}
...
function buildRedactor(sensitiveMap) {
  ...
  const replacers = values.map((v) => ({ re: new RegExp(escapeRegExp(v), "g"), replacement: "***REDACTED***" }));
    ...
    console[methodName] = (...args) => {
      try {
        const safe = args.map((a) => {
          ...
          try { return JSON.parse(redact(JSON.stringify(a))); } catch { return a; }
        });
        orig(...safe);
      } catch {
        orig(...args);
...
export function validateEnvAndInstallRedaction(options = {}) {
  ...
  const strict = strictInProd ? isProd || asBool(process.env.ENV_VALIDATION_STRICT) : false;
  ...
  if (!RPC_HTTP_ENDPOINTS && !SOLANA_RPC_URLS && !SOLANA_RPC_URL) {
    errors.push("At least one of RPC_HTTP_ENDPOINTS, SOLANA_RPC_URLS, or SOLANA_RPC_URL must be set");
  }

  const MONGODB_URI = process.env.MONGODB_URI;
  if (!MONGODB_URI) errors.push("MONGODB_URI is required");
  else if (!isMongoUri(MONGODB_URI)) errors.push("MONGODB_URI must start with mongodb:// or mongodb+srv://");

  const WALLET_ENCRYPTION_KEY = process.env.WALLET_ENCRYPTION_KEY;
  if (strict && !WALLET_ENCRYPTION_KEY) errors.push("WALLET_ENCRYPTION_KEY is required in production");
  ...
  if (ENABLE_PRIVATE_RELAY) {
    if (!process.env.PRIVATE_RELAY_ENDPOINT) errors.push("PRIVATE_RELAY_ENDPOINT is required when ENABLE_PRIVATE_RELAY=true");
    if (!process.env.PRIVATE_RELAY_API_KEY) errors.push("PRIVATE_RELAY_API_KEY is required when ENABLE_PRIVATE_RELAY=true");
  }

  // Optional relay preference
  if (process.env.PRIVATE_RELAY_PREFER != null && process.env.PRIVATE_RELAY_PREFER !== "") {
    const v = String(process.env.PRIVATE_RELAY_PREFER).toLowerCase();
    const allowed = new Set(["jito","bloxroute","generic"]);
    if (!allowed.has(v)) errors.push("PRIVATE_RELAY_PREFER must be one of: jito, bloxroute, generic");
  }

  // Optional: explicit relay vendor selection
  if (process.env.PRIVATE_RELAY_VENDOR != null && process.env.PRIVATE_RELAY_VENDOR !== "") {
    const v = String(process.env.PRIVATE_RELAY_VENDOR).toLowerCase();
    const allowed = new Set(["auto","jito","bloxroute","flashbots","generic"]);
    if (!allowed.has(v)) errors.push("PRIVATE_RELAY_VENDOR must be one of: auto, jito, bloxroute, flashbots, generic");
  }
  ...
  [
    "PRIORITY_FEE_CAP_T1_MS","PRIORITY_FEE_CAP_T2_MS","PRIORITY_FEE_CAP_T3_MS",
    "PRIORITY_FEE_CAP_LOW","PRIORITY_FEE_CAP_MID","PRIORITY_FEE_CAP_HIGH","PRIORITY_FEE_CAP_MAX",
    "PRIORITY_FEE_TARGET_PCT","TIP_FEEDBACK_WINDOW","TIP_TARGET_LATENCY_MS","TIP_HEADROOM"
  ].forEach((k) => { asNumberInRange(k, process.env[k], { min: 0, max: 1e9, errors }); });

  // RPC racing
  ["RPC_SEND_TIMEOUT_MS","RPC_STAGGER_STEP_MS","RPC_INTER_WAVE_DELAY_MS"].forEach((k) => {
    asNumberInRange(k, process.env[k], { min: 1, max: 60000, errors });
  });

  // Compute budget jitter
  ["COMPUTE_UNIT_PRICE_JITTER_PCT","COMPUTE_UNIT_LIMIT_JITTER_PCT","COMPUTE_UNIT_LIMIT_BASE"].forEach((k) => {
    asNumberInRange(k, process.env[k], { min: 0, max: 1e9, errors });
  });

  // Jito (optional)
  ["JITO_SLOTS_AHEAD","JITO_RETRIES","JITO_RETRY_DELAY_MS","JITO_MAX_WAIT_MS"].forEach((k) => {
    asNumberInRange(k, process.env[k], { min: 0, max: 1e9, errors });
  });
  ...
  if (process.env.ADAPTIVE_SLIPPAGE_ENABLED != null) {
    ...
    if (!ok.includes(v)) errors.push("ADAPTIVE_SLIPPAGE_ENABLED must be one of: 1,true,yes,on,0,false,no,off");
  }

  // Liquidity delta & Pre-LP knobs (optional)
  asNumberInRange("DELTA_MAX_PRICE_IMPACT_PCT", process.env.DELTA_MAX_PRICE_IMPACT_PCT, { min: 0, max: 100, errors });
  asNumberInRange("LIQ_DELTA_PROBE_SOL", process.env.LIQ_DELTA_PROBE_SOL, { min: 0.001, max: 1000, errors });
  asNumberInRange("LIQ_DELTA_MIN_IMPROV_PCT", process.env.LIQ_DELTA_MIN_IMPROV_PCT, { min: 0, max: 1000, errors });
  asNumberInRange("DELTA_MIN_ROUTE_AGE_MS", process.env.DELTA_MIN_ROUTE_AGE_MS, { min: 0, max: 1e9, errors });
  asNumberInRange("PRELP_CONFIDENCE_MIN", process.env.PRELP_CONFIDENCE_MIN, { min: 0, max: 100, errors });
  asNumberInRange("PRELP_COOL_MS", process.env.PRELP_COOL_MS, { min: 0, max: 1e9, errors });
