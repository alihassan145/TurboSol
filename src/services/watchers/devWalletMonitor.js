...
export async function addDevWalletToMonitor(chatId, addressBase58) {
  const k = `${chatId}:set`;
  if (!monitors.has(k)) monitors.set(k, { addrs: new Set(), last: new Map(), interval: null });
  monitors.get(k).addrs.add(addressBase58);
}

export async function startDevWalletMonitor(chatId, { intervalMs = 400, onTx, autoSnipeOnTokenProgram = true } = {}) {
  const k = `${chatId}:set`;
  const entry = monitors.get(k) || { addrs: new Set(), last: new Map(), interval: null };
  ...
  entry.interval = setInterval(async () => {
    for (const addr of entry.addrs) {
      try {
        const sigs = await conn.getSignaturesForAddress(new PublicKey(addr), { limit: 5 });
        ...
        for (const s of sigs) {
          ...
          if (autoSnipeOnTokenProgram && await shouldTriggerAutoSnipe(chatId, s)) {
...
async function extractMintFromTx(connection, sigInfo) {
  try {
    ...
    for (const log of logs) {
      ...
      const createMatch = log.match(/Program\s+TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA\s.*Initialize.*Mint.*account.*([A-Za-z0-9]{32,44})/);
